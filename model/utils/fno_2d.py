...
class SpectralConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, modes1, modes2):
        ...
        self.modes1 = modes1 # Number of Fourier modes to multiply, at most floor(N/2) + 1 # for symmetry property , k
        self.modes2 = modes2
    ...
    def forward(self, x, size=None):
        ...
        x_ft = torch.fft.rfftn(x, dim=[2,3], norm="ortho") # DFT of the 2d picture, decomposed to orthonormal vectors

        # Multiply relevant Fourier modes
        out_ft = torch.zeros(batchsize, self.out_channels, size, size//2 + 1, device=x.device, dtype=torch.cfloat)
        out_ft[:, :, :self.modes1, :self.modes2] = \
            compl_mul2d(x_ft[:, :, :self.modes1, :self.modes2], self.weights1) # weighted low frequency modes
        out_ft[:, :, -self.modes1:, :self.modes2] = \
            compl_mul2d(x_ft[:, :, -self.modes1:, :self.modes2], self.weights2) # weighted high frequency modes
...
class SimpleBlock2d(nn.Module):
    def __init__(self, in_dim, out_dim, domain_size, modes1, modes2, width): # assumes square domain
        ...
        self.width_list = [width*2//4, width*3//4, width*4//4, width*4//4, width*5//4] # richer feature space
        self.size_list = [domain_size] * 5
        self.grid_dim = 2

        self.fc0 = nn.Linear(in_dim+self.grid_dim, self.width_list[0])
        # with layer depth and feature space increasing, the number of modes decreases for filtering, keeps computational efficiency
    ...
    def forward(self, x):
        ...
        batchsize = x.shape[0] # input tensor shape: (batchsize, x,y,1)
        size_x, size_y= x.shape[1], x.shape[2] # picture dimensions 
        grid = self.get_grid(size_x, batchsize, x.device)
        size_list = self.size_list

        # grid shape (batchsize,2,S,S) -> permute (batchsize, S, S, 2) -> concated to (batchsize, S, S, 3)
        x = torch.cat((x, grid.permute(0, 2, 3, 1)), dim=-1)
        x = self.fc0(x) # -> (batchsize, S, S, width_list[0])
        x = x.permute(0, 3, 1, 2) # permute back to (batchsize, width_list[0], S, S)

        x1 = self.conv0(x, size_list[1]) # spectral conv -> (batchsize, width_list[1], S, S)
        ...
        x = x1 + x2
        x = F.selu(x) # residual connection -> activation
        ...
        # feel free to add/reduce the fc layers
        x = x.permute(0, 2, 3, 1) # permute to (batchsize, S, S, width_list[4])
        x = self.fc1(x)
        x = F.selu(x)
        x = self.fc2(x)
        x = F.selu(x)
...
class CustomizedBlock2d(nn.Module):
    def __init__(self, in_dim, out_dim, domain_size, modes1, modes2, width): # assumes square domain
        super(CustomizedBlock2d, self).__init__()
    ...
    def forward(self, x):
        ...
        x1 = self.conv0(x, size_list[1]) # spectral conv -> (batchsize, width_list[1], S, S)
        x = F.selu(x1) # residual connection -> activation
...
class CustomizedNet2d(nn.Module):
    def __init__(self, in_dim, out_dim, domain_size, modes, width):
        super(CustomizedNet2d, self).__init__()
        self.conv1 = CustomizedNet2d(in_dim, out_dim, domain_size, modes, modes, width)
